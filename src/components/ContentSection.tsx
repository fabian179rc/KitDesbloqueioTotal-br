import React from 'react';
import { motion } from 'framer-motion';
import { PackageOpen, CheckCircle2 } from 'lucide-react';

const temas = [
  { emoji: '💰', label: 'Desbloqueio Financeiro' },
  { emoji: '❤️', label: 'Desbloqueio Amoroso' },
  { emoji: '🏠', label: 'Proteção do Lar' },
  { emoji: '👨‍👩‍👧', label: 'Harmonia Familiar' },
  { emoji: '🧠', label: 'Paz Mental' },
  { emoji: '🌙', label: 'Limpeza Energética' },
  { emoji: '🌿', label: 'Cura Ancestral' },
  { emoji: '💼', label: 'Prosperidade no Trabalho' },
  { emoji: '⚡', label: 'Corte de Laços Tóxicos' },
  { emoji: '🔮', label: 'Proteção Contra Inveja' },
  { emoji: '🌊', label: 'Renovação de Caminhos' },
  { emoji: '☀️', label: 'Atração de Abundância' },
];

const situacoes = [
  { emoji: '💸', label: 'Dinheiro que some sem explicação' },
  { emoji: '💔', label: 'Relacionamentos que não evoluem' },
  { emoji: '😰', label: 'Ansiedade e sensação de peso constante' },
  { emoji: '🚧', label: 'Oportunidades que sempre escapam' },
  { emoji: '🏚️', label: 'Lar com energia pesada e conflitos frequentes' },
  { emoji: '🔄', label: 'Ciclos negativos que se repetem' },
];

const incluye = [
  '📝 Orientação do Dia — O que fazer e por quê, explicado de forma clara antes de cada ritual',
  '🎯 Ritual Específico — Passo a passo com elementos simples que você já tem em casa',
  '✍️ Decreto de Poder — O texto exato para declarar sua libertação espiritual em voz alta',
  '🗒️ Checklist de Execução — Confirme cada etapa e nunca perca o fio do protocolo',
  '🏅 Confirmação de Fechamento — Encerre o dia com clareza de que o trabalho foi concluído',
];

export function ContentSection() {
  return (
    <section className="py-10 md:py-14 bg-transparent">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="text-center mb-8">
          <span className="inline-flex items-center gap-2 px-6 py-2.5 mb-5 rounded-full border border-white/40 bg-[#4A553F] text-white font-semibold tracking-[0.18em] uppercase text-xs">
            <PackageOpen className="w-4 h-4 text-[#d4a017]" />
            O que contém?
          </span>
          <h2 className="font-heading font-bold text-[#2f3a2c] text-3xl md:text-5xl leading-tight mb-4">
            O QUE CONTÉM O KIT{' '}
            <span className="text-[#5C6851]">DESBLOQUEIO TOTAL?</span>
          </h2>
          <p className="text-lg md:text-xl font-bold text-[#5C6851] max-w-2xl mx-auto">
            Tudo para seu protocolo de 7 dias, com método, clareza e proteção real. 💪
          </p>
        </div>

        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-white/60 border border-amber-100 rounded-3xl p-5 md:p-7 shadow-sm mb-6 text-center"
        >
          <p className="text-slate-700 text-[15px] md:text-lg leading-relaxed">
            📚 O{' '}
            <span className="font-bold text-[#5C6851]">
              Kit Desbloqueio Total
            </span>{' '}
            reúne mais de{' '}
            <span className="font-semibold text-[#2f3a2c]">80 páginas</span>{' '}
            com o método completo para identificar, negociar e encerrar
            dívidas espirituais que estão bloqueando sua prosperidade e paz,
            organizado em{' '}
            <span className="font-semibold text-[#2f3a2c]">7 dias de protocolo</span>,
            com{' '}
            <span className="font-semibold text-[#2f3a2c]">5 bônus inclusos</span>.
          </p>
        </motion.div>

        <div className="mb-8">
          <h3 className="font-heading font-bold text-[#2f3a2c] text-xl md:text-2xl mb-4 text-center">
            🎯 Cobre as áreas mais importantes da sua vida espiritual
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {temas.map((t, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, scale: 0.95 }}
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.03 }}
                className="flex items-center gap-2.5 bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-sm"
              >
                <span className="text-xl flex-shrink-0" aria-hidden="true">
                  {t.emoji}
                </span>
                <span className="font-semibold text-[#2f3a2c] text-sm md:text-[15px] leading-tight">
                  {t.label}
                </span>
              </motion.div>
            ))}
          </div>
        </div>

        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-[#4A553F] rounded-3xl p-5 md:p-7 shadow-sm mb-8 text-center"
        >
          <h3 className="font-heading font-bold text-white text-xl md:text-2xl mb-5">
            🏠 Indicado para quem vive situações como:
          </h3>
          <div className="flex flex-wrap justify-center gap-3 mb-6">
            {situacoes.map((r, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-2 bg-white/10 border border-white/20 text-white rounded-full px-4 py-2 text-sm font-semibold"
              >
                <span aria-hidden="true">{r.emoji}</span>
                {r.label}
              </span>
            ))}
          </div>
          <p className="text-[#e6ddc7] text-sm md:text-base leading-relaxed max-w-2xl mx-auto">
            ⚖️ Guia prático baseado em princípios espirituais universais,
            adaptável à sua crença e tradição religiosa — Espiritismo,
            Catolicismo, Umbanda, Candomblé ou qualquer outra.
          </p>
        </motion.div>

        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-white border border-amber-100 rounded-3xl p-5 md:p-7 shadow-lg shadow-black/5"
        >
          <h3 className="font-heading font-bold text-[#2f3a2c] text-2xl md:text-3xl mb-5 text-center">
            ✨ Cada dia do protocolo inclui:
          </h3>
          <div className="grid sm:grid-cols-2 gap-2 sm:gap-3 max-w-2xl mx-auto">
            {incluye.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-2 sm:gap-3 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2 sm:px-4 sm:py-3"
              >
                <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 flex-shrink-0" />
                <span className="font-semibold text-[#2f3a2c] text-[12px] sm:text-[15px] leading-tight">
                  {item}
                </span>
              </div>
            ))}
          </div>
        </motion.div>
      </div>
    </section>
  );
}
